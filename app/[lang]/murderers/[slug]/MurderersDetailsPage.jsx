...
const MurderersDetailsPage = ({ slug, lang }) => {
  const { data: murderer, error, isLoading } = useGetMurderersBySlugQuery(
    { slug, lang },
    { skip: !slug }
  );
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="flex flex-col items-center md:items-start">
          ...
          <div className="w-40 md:w-48 mt-2 p-1 text-left">
            <h1 className="text-lg font-bold text-gray-800">{murderer?.name || "Unknown"}</h1>
            <p className="text-gray-600 text-sm">{murderer?.occupation || "N/A"}</p>
            <p className="text-gray-500 text-sm">{murderer?.organization || "N/A"}</p>
            <p className="text-gray-500 text-sm">{murderer?.designation || "N/A"}</p>
          </div>
        </div>
        ...
        <div className="col-span-2 p-2 bg-[#f6f6f6] rounded-md shadow-sm">
          ...
          {murderer?.incident_details && (
            <div className="mt-6 p-4 bg-[#e7e7e7] rounded-md shadow-sm">
              ...
              <p className="text-gray-700 mt-2">{murderer.incident_details}</p>
            </div>
          )}
          ...
          {murderer?.biography && (
            <div className="mt-6 p-4 bg-[#e7e7e7] rounded-md shadow-sm">
              ...
              <p className="text-gray-700 mt-2">{murderer.biography}</p>
            </div>
          )}
        </div>
      </div>
